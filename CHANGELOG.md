...
- Adds new `analyze` subcommand to print the contents of the current project's blueprint to stdout (closes #10)
...
- Adds support for UTF-8 and ISO-8859-1 source files (closes #22)

## 0.29.2

### Fixes
- Uses correct library name for custom recursive filesets collected into the blueprint
- Fixes incorrect syntax generated by `get` command when a `.` is used
- Removes debug print statement for VHDL directives
- Configuration files found directly under `.orbit` directories now properly analyze relative paths by starting one directory up (relative to the directory where the `.orbit` directory exists)
...
- Fixes VHDL packages not appearing under VHDL contexts when running `orbit tree -e all`
