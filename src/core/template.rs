...
impl Template {
    ...
    fn gather_files(
        ...
        let result = ignore::WalkBuilder::new(src)
            ...
            .hidden(false)
            .build();
        let paths: Vec<(bool, PathBuf)> = result
            .into_iter()
            .filter_map(|f| {
                if f.is_ok() {
                    Some((f.as_ref().unwrap().path().is_dir(), f.unwrap().into_path()))
                } else {
                    None
                }
            })
...
{2:->46}{2:->10}\n",
        ...
        let s = all.iter().fold(String::new(), |x, y| {
            x + &format!(
                "{:<46}{:<10}\n",
                // pretty-print the filepath as relative with forward-slashes
                &PathBuf::standardize(filesystem::remove_base(&root, &y))
                    .display()
                    .to_string(),
                // denote if the file was ignored or not
                {
                    if imports_set.contains(y) == false {
                        "y"
                    } else {
                        ""
                    }
...
"#;
        ...
        assert_eq!(
            ...
            Template {
                ...
                ignores: vec!["build/".to_string(), "extra/".to_string(),],
            }
        );
    }
}
