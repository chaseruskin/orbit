...
- Orbit helps you quickly survey what projects exist in the catalog with `orbit search`. This will return a list of all projects available in the catalog, their latest version, their status (installed, downloaded, available), and its unique identifier.

- Further refining your search to get more information about a particular project can be done with `orbit info`. This command can display data pulled from the project's manifest, what design units it contains, the different versions of the project, and other data related to that project.
...
- Using any design units from existing project in your current project can be quickly achieved using `orbit get`. This command will return the necessary HDL code snippets to correctly instantiate a design unit. Simply adding the external project's name and version to the current project's `[dependencies]` table within the `Orbit.toml` file will allow Orbit to record the information necessary to correctly reference the design unit's within that project.
...
- Securing the current state of the project as a release (such as version 1.0.0) for others to find is done using `orbit publish`. This command verifies the current project's state is valid in the context of Orbit and then installs it to your catalog as well as posts the project's manifest to any of its configured channels. By posting the project to a channel, any other users who also have the same channel configured will be able to see the newly available project.
