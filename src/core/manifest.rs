...
use crate::util::sha256::{self, Sha256Hash};
...
impl IpManifest {
    ...
    pub fn display_information(&self, state: &IpState) -> String {
        ...
        let size = match &state {
            ...
            _ => {
                let sz = crate::util::filesystem::compute_size(
                    &self.manifest.get_path().parent().unwrap(),
                    crate::util::filesystem::Unit::MegaBytes,
                )
                .unwrap();
                format!("{:.2}", sz)
            }
...
{}",
            ...
            self.get_version(),
            url,
            crate::util::filesystem::compute_size(
                &self.manifest.get_path().parent().unwrap(),
                crate::util::filesystem::Unit::MegaBytes
            )
            .unwrap(),
        ...
        Ok(Self {
            ...
            units: match table.get("units") {
                Some(i) => match i.as_array() {
                    Some(arr) => {
                        let result: Result<Vec<_>, IdentifierError> = arr
                            .into_iter()
            ...
            match item.as_table() {
                ...
                None => {
                    return Err(AnyError(format!(
                        "expects key 'dependencies' to be a toml table"
                    )))?
                }
            }
        } else {
            ...
            for (library, l_item) in v_item.as_table().unwrap_or(&toml_edit::Table::default()) {
                ...
                for (name, n_item) in l_item.as_table().unwrap_or(&toml_edit::Table::default()) {
                    ...
                    let version = match n_item.as_str() {
                        ...
                        None => {
                            return Err(FromTomlError::ExpectingString(format!(
                                "{}.{}.{}",
                                vendor, library, name
                            )))?
                        }
                ...
                if switch == true {
                    return Err(AnyError(format!(
                        "partial ip pkgid key '{}.{}.' in dependencies table",
                        vendor, library
                    )))?;
                }
            }
...
impl IpManifest {
    ...
    pub fn detect_all(path: &PathBuf) -> Result<Vec<Self>, Box<dyn std::error::Error>> {
        Manifest::detect_all(path, IP_MANIFEST_FILE, true)?
            .into_iter()
            .map(|f| IpManifest::from_manifest(f))
            .collect()
    }
    ...
    pub fn detect_available(path: &PathBuf) -> Result<Vec<Self>, Box<dyn std::error::Error>> {
        Manifest::detect_all(path, IP_MANIFEST_PATTERN_FILE, false)?
            .into_iter()
            .map(|f| IpManifest::from_manifest(f))
            .collect()
    }
    ...
    pub fn compute_checksum(&self) -> Sha256Hash {
        ...
        let checksum =
            crate::util::checksum::checksum(&ip_files, &std::env::current_dir().unwrap());
    ...
    pub fn stash_size(&mut self) -> Result<(), Fault> {
        ...
        let tbl = self.get_manifest_mut().get_mut_doc()["ip"]
            .as_table_mut()
            .unwrap();
    ...
    pub fn read_checksum_proof(&self) -> Option<Sha256Hash> {
        ...
        if sum_file.exists() == false {
            ...
            match std::fs::read_to_string(&sum_file) {
                Ok(text) => match sha256::Sha256Hash::from_str(&text.trim()) {
                    Ok(sha) => Some(sha),
                    Err(_) => None,
                },
                Err(_) => None,
    ...
    pub fn insert_dependency(&mut self, pkgid: PkgId, ver: AnyVersion) -> Option<AnyVersion> {
        if self
            ...
            .get_doc()
            .as_table()
            .contains_table(DEPENDENCIES_KEY)
            == false
        {
            self.get_manifest_mut().get_mut_doc()[DEPENDENCIES_KEY] =
                toml_edit::Item::Table(Table::new());
        }
        let prev_value = self.ip.deps.inner_mut().remove(&pkgid);
        self.get_manifest_mut().get_mut_doc()[DEPENDENCIES_KEY]
            [&pkgid.get_vendor().as_ref().unwrap().to_string()]
            [&pkgid.get_library().as_ref().unwrap().to_string()][&pkgid.get_name().to_string()] =
            toml_edit::value(&ver.to_string());

        self.get_manifest_mut().get_mut_doc()[DEPENDENCIES_KEY]
            [&pkgid.get_vendor().as_ref().unwrap().to_string()]
            .as_inline_table_mut()
            .map(|f| f.set_dotted(true));
    ...
    pub fn create(
        ...
        ip_man
            .get_manifest_mut()
            .write("ip", "name", pkgid.get_name());
        ip_man
            .get_manifest_mut()
            .write("ip", "library", pkgid.get_library().as_ref().unwrap());
        ip_man
            .get_manifest_mut()
            .write("ip", "vendor", pkgid.get_vendor().as_ref().unwrap());
        // save the manifest
    ...
    fn from_manifest(m: Manifest) -> Result<Self, Box<dyn Error>> {
        Ok(IpManifest {
            ip: Self::wrap_toml(&m, IpToml::from_toml(&m.get_doc().as_table()))?,
            manifest: m,
        })
    }
    ...
    pub fn from_store(
        ...
        pkgid: &PkgId,
        _version: &AnyVersion,
    ) -> Result<Option<IpManifest>, Fault> {
    ...
    pub fn stash_units(&mut self) -> () {
        ...
        tbl.insert(
            "units",
            toml_edit::Item::Value(toml_edit::Value::Array(Array::new())),
        );
...
"#;
        ...
        map.insert(
            ...
            AnyVersion::Specific(PartialVersion::new().major(1).minor(0).patch(0)),
        );
        map.insert(
            PkgId::from_str("ks-tech.util.toolbox").unwrap(),
            AnyVersion::Specific(PartialVersion::new().major(2)),
        );
        map.insert(
            PkgId::from_str("c-rus.eel4712c.lab1").unwrap(),
            AnyVersion::Specific(PartialVersion::new().major(4).minor(2)),
        );
