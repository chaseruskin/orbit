...
impl Subcommand<Context> for OrbitSubcommand {
    fn interpret<'c>(cli: &'c mut Cli<Memory>) -> cli::Result<Self> {
        match cli
            .select(&[
                ...
                "analyze", "init", "tree", "info", "b", "env", "config", "remove", "read", "doc",
                "clean",
            ])?
            .as_ref()
        {
            ...
            "clean" => Ok(OrbitSubcommand::Clean(Clean::interpret(cli)?)),
            "init" => Ok(OrbitSubcommand::Init(Init::interpret(cli)?)),
            "doc" => Ok(OrbitSubcommand::Doc(Doc::interpret(cli)?)),
    ...
    fn execute(self, context: &Context) -> proc::Result {
        match self {
            ...
            OrbitSubcommand::Doc(sub) => sub.execute(context),
            ...
            OrbitSubcommand::Clean(sub) => sub.execute(context),
        }
    }
}
