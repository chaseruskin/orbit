use crate::util::{
    ...
    filesystem::Standardize,
};
use colored::Colorize;
use std::{io::Write, path::PathBuf};
use toml_edit::{Array, ArrayOfTables, Document, Formatted, Item, Table, Value};
...
pub trait FromToml {
        ...
        <T as std::str::FromStr>::Err: std::error::Error,
    {
        ...
        match result {
            Ok(r) => Ok(Some(r)),
            Err(e) => Err(FromTomlError::BadParse(
                s.to_string(),
                table.get(s).unwrap().as_str().unwrap().to_string(),
                e.to_string(),
            )),
        }
    }
...
impl std::fmt::Display for FromTomlError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::BadParse(key, value, err) => write!(
                f,
                "failed to parse value '{}' for key '{}' due to {}",
                value.yellow(),
                key.yellow(),
                err
            ),
            Self::ExpectingStringArray(key) => {
                write!(f, "key '{}' expects an array of strings", key)
            }
        }
    }
}
...
impl Config {
    ...
    pub fn collect_as_str<'a>(&'a self, table: &str, key: &str) -> Result<Vec<&'a str>, Fault> {
        Ok(self
            .collect_as_item(Some(table), key, &Item::is_str, "string")?
            .into_iter()
    ...
    pub fn collect_as_array_of_str<'a>(
        &'a self,
        table: &str,
        key: &str,
    ) -> Result<Vec<(&'a str, &PathBuf)>, Fault> {
        let mut all_items = Vec::<(&str, &PathBuf)>::new();
        let arrays: Vec<(&Array, &PathBuf)> = self
            .collect_as_item(Some(table), key, &Item::is_array, "array")?
            .into_iter()
            .map(|f| (f.0.as_array().unwrap(), f.1))
            .collect();
        for arr in arrays {
            for item in arr.0 {
    ...
    /// Returns `Vec::new()` if the entry does not exist anywhere.
    pub fn collect_as_array_of_tables<'a>(
        &'a self,
        key: &str,
    ) -> Result<Vec<(&ArrayOfTables, &PathBuf)>, Fault> {
        Ok(self
            .collect_as_item(None, key, &Item::is_array_of_tables, "array of tables")?
            .into_iter()
            .map(|f| (f.0.as_array_of_tables().unwrap(), f.1))
            .collect())
    }
    ...
    pub fn collect_as_tables<'a>(&'a self, key: &str) -> Result<Vec<(&Table, &PathBuf)>, Fault> {
        Ok(self
            .collect_as_item(None, key, &Item::is_table, "table")?
            .into_iter()
            .map(|f| (f.0.as_table().unwrap(), f.1))
            .collect())
    }
    ...
    fn collect_as_item<'a>(
        &'a self,
        table: Option<&str>,
        key: &str,
        eval: &dyn Fn(&Item) -> bool,
        item_name: &str,
    ) -> Result<Vec<(&Item, &PathBuf)>, Fault> {
            ...
            match inc.access(table, key) {
                Some(item) => {
                    ...
                    if eval(item) {
                        ...
                        return Err(ConfigError::BadItem(
                            format!(
                                "{}",
                                PathBuf::standardize(self.get_root().join(CONFIG_FILE)).display()
                            ),
                            item_name.to_owned(),
                            format!(
                                "{}{}",
...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::BadItem(file, item, key) => write!(
                f,
                "configuration {}: expecting toml {} for key '{}'",
                file, item, key
            ),
        }
    }
...
mod test {
    ...
    fn access_includes() {
        ...
        assert_eq!(
            cfg.collect_as_str("core", "editor").unwrap(),
            vec!["vim", "code"]
        );
    ...
    fn collect_items() {
        ...
        let items: Vec<&str> = cfg
            .collect_as_item(Some("core"), "editor", &Item::is_str, "string")
            .unwrap()
            .into_iter()
            .map(|f| f.0.as_str().unwrap())
            .collect();
